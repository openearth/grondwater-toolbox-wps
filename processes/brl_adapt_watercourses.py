...
def createpointer(modeltmpdir, waters_id, modeldir, tmpdir): # input_config: een lijst met per watersid een lijst met laag, parameter, waarde
    ...
    acrs = '28992'
    rivers = os.path.join(modeldir,'riv/hoofdwater/PEILH_1998_2006_mean.IDF')

    drawn_extent = os.path.join(tmpdir,f'{waters_id}_extent_rd.geojson')  
    print('drawn_extent',drawn_extent)
    ...
    data_proj.to_file(ashp,crs='EPSG:{c}'.format(c=acrs))
    ...
    for i in to_cleanup:
        ...
        os.unlink(ashp)
    print('voor retourneren pointer')    
    return pointer
    
def adjustrivpackage(modeltmpdir, watersId, lstjsonmeasures, modeldir, tmpdir):
    # Possible adjusted parameters: stage, riverbed, conductance
    # Make a copy of the original files of these parameters
    print('adjust riv package')
    ...
    for lm in lstjsonmeasures:
        ...
        print('condbeddif_l1',rivbeddif_l1)
